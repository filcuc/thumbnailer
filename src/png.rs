...
impl Chunk {
    pub fn new_text<S: Into<String>>(keyword: &str, text: S) -> Result<Chunk, ()> {
        let text = text.into();
...
impl Png {
    ...
    pub fn decode(input: &mut dyn std::io::Read) -> Result<Vec<Chunk>, ()> {
    ...
    pub fn encode(output: &mut dyn std::io::Write, chunks: &Vec<Chunk>) -> Result<(), ()> {
        output
            .write_all(Png::PNG_SIGNATURE.as_ref())
            .map_err(|_e| ())?;
        for c in chunks {
            let mut crc = CRC::new();
...
mod tests {
    ...
    fn test_png_encoder() {
        ...
        {
            fn read_all_data(path: &PathBuf) -> Vec<u8> {
                let mut data: Vec<u8> = Default::default();
                let mut file = std::fs::File::open(path).unwrap();
                file.read_to_end(&mut data).unwrap();
                data
            };
            assert_eq!(read_all_data(&input_path), read_all_data(&output_path));
        }
    }
}
